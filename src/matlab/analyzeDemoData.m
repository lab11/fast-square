...
for ii=1:length(restarts)-1
    ...
    for jj=1:num_steps
        ...
        cur_step_data(imag(cur_step_data) < 0) = cur_step_data(imag(cur_step_data) < 0) + 65536*1i;
        cur_step_data = real(cur_step_data)*65536+imag(cur_step_data);
        freq_data(ii,jj,:) = 1i*cur_step_data(1:4) + cur_step_data(5:end);
    end
end
