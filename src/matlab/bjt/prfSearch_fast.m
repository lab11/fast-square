...
cur_iq_data_temp = cat(3,cur_iq_data,zeros(size(cur_iq_data,1),size(cur_iq_data,2),fft_len-size(cur_iq_data,3)));
...
cur_iq_fft_abs = squeeze(abs(fft(cur_iq_data_temp(prf_anchor,:,:),[],3))).';
